...
def load_image(image_path: str | Path):
    ...
    image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB); image = cv2.resize(image, IMAGE_SIZE); image = image.astype("float32") / 255.0
...
def main():
    ...
    model = tf.keras.models.load_model(args.model); image = load_image(args.image)
    probabilities = model.predict(image)[0]; class_index = int(np.argmax(probabilities))
    print(f"Predicted class: {args.classes[class_index]}"); print(f"Confidence: {probabilities[class_index]:.4f}")
